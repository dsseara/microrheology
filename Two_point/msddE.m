function [msd1 flor] = msddE(msd2p,rmin,rmax,dtmax,a,lfit2,width)
%
...
for i=1:ndt
    ...
    if lfit2 == 0  
        ...
        msd1(i,3) = (4/(3*a))*sum(perpr)/length(perpr);
                ...
                % standard error of the average from standard deviation of
                % points
        msd1(i,7) = sqrt(sum(((2/(3*a))*parr-msd1(i,2)).^2)/max((length(parr)-1),1))/sqrt(length(parr));
        msd1(i,8) = sqrt(sum(((4/(3*a))*perpr-msd1(i,3)).^2)/max((length(perpr)-1),1))/sqrt(length(parr));
    else
...
if width > 0
    ...
    % NOTE: done outside of msdd by call_two_fitting_methods, since that 
    % calls msdd with only one tau, so get lots of warnings about the 
    % fitting of only one point.
end
